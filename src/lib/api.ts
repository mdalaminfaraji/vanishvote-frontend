...
// Vote on a poll
export async function voteOnPoll(pollId: string, voteData: VoteData): Promise<Poll> {
  try {
...
export async function addReaction(pollId: string, reactionData: ReactionData): Promise<{ pollId: string; reactions: { trending: number; likes: number } }> {
  try {
